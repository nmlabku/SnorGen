...
void Sequence::parserHTTPMethod(Sequence* p_cpResultSequence)
{
	...
	//end set
	cContentTemp.reset();
	cContentTemp.concatenateChars((char*)" ", strlen(" "));//공백을 추가?
	iEnd = this->cContent.isInclude(iSart, &cContentTemp); //매칭된 마지막 오프셋 리턴
	if (iEnd == 0) return;
	iEnd -= cContentTemp.getLength();//Method 부분은 제일 앞에 나오기때문에 이런식으로 함
		...
		for ( go = head->next;  go != NULL;  go = go->next )				// 모든 flow 순회
		{
			...
			for ( PktGO = go->headPkt;  PktGO!=NULL;  PktGO = PktGO->next) //packet
			{
				...
				if(go->isForwardDirection(&PktGO->pkt)) //forward packet
				{
					...
					{
						cSequenceTemp.setSequenceID(uiSequenceID++);//SeqID 0부터 쭉쭉 증가..
						//------------------------------------------------
						cSuspectTemp.reset();
						...
						this->insert(&cSequenceTemp);// SequenceVector는 STL 링크드 리스트이기 때문에 insert만 해줘도 자동으로 노드 추가
					}
				}
